    ...
    def transform(self, _ctx: context.Context, modules: List[statements.ModSubmodStatement]):
        for mod_stmt in modules:
            ...
            for obj_kywd in AMM_OBJ_NAMES:
                ...
                amm_prefix = [key for key, (name, _rev) in mod_stmt.i_prefixes.items() if name == MODULE_NAME]
...
class DtnmaRefsPlugin(plugin.PyangPlugin):
    ...
    def setup_ctx(self, ctx: context.Context):
        ...
        ari_keywords = [keyword for keyword, (arg_type, _subspec) in grammar.stmt_map.items() if arg_type == "ARI"]

        # reference checking after all module-level resolution
        statements.add_validation_fun("reference_4", ari_keywords, self._chk_ari)

        # register our error codes
        error.add_error_code(
            "ADM_DEREFERENCE_FAILED",
            3,  # minor error
    ...
    def _chk_ari(self, ctx: context.Context, stmt: statements.Statement):
        ...
        if obj is None:
            error.err_add(ctx.errors, stmt.pos, "ADM_DEREFERENCE_FAILED", (stmt.arg,))
