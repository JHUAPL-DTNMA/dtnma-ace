...
def pyang_plugin_init():
    ...
    grammar.add_to_stmts_rules(["grouping"], [(name, "*") for name in AMM_GROUPING_NAMES])
    for name in AMM_GROUPING_NAMES:
        statements.add_data_keyword(name)

    statements.add_validation_fun("grammar", ["namespace"], _stmt_check_namespace)
    statements.add_validation_fun("grammar", ["module"], _stmt_check_module_enums)
    statements.add_validation_fun("grammar", AMM_OBJ_NAMES, _stmt_check_obj_enum)
    statements.add_validation_fun("grammar", ["module", "submodule"], _stmt_check_module_objs)
    statements.add_validation_fun("grammar", [(MODULE_NAME, "int-labels")], _stmt_check_intlabels)
    ...
    statements.add_validation_fun("unique_name", ["module"], _stmt_check_enum_unique)
    statements.add_validation_fun(
        "grammar",
        AMM_OBJ_NAMES
        + (
...
MODULE_EXTENSIONS = (
    ...
    Ext(
        "typedef",
        "identifier",
        subs=(OBJ_SUBS_PRE + type_use("typedef")),
    ),
    Ext(
        "ident",
        "identifier",
...
def _stmt_check_namespace(ctx: context.Context, stmt: statements.Statement):
    ...
    if not isinstance(ns_ref, ReferenceARI) or ns_ref.ident.type_id is not None or ns_ref.ident.obj_id is not None:
        error.err_add(ctx.errors, stmt.pos, "AMM_MODULE_NS_ARI", (stmt.arg))
    ...
    if ns_ref and ns_ref.ident.module_name != module_name.casefold():
        error.err_add(ctx.errors, stmt.pos, "AMM_MODULE_NAME_NS", (module_name, stmt.arg))
...
def _stmt_check_ari_import_use(ctx: context.Context, stmt: statements.Statement):
    ...
    mod_map = {name: key for key, (name, _rev) in mod_stmt.i_prefixes.items()}
...
def _stmt_check_module_enums(ctx: context.Context, stmt: statements.Statement):
    ...
    if not enum_stmt:
        error.err_add(ctx.errors, stmt.pos, "AMM_MODEL_ENUM", (stmt.arg))

    org_stmt = stmt.search_one("organization")
    enum_stmt = org_stmt.search_one((MODULE_NAME, "enum")) if org_stmt else None
    if not enum_stmt:
        error.err_add(ctx.errors, stmt.pos, "AMM_ORG_ENUM", (stmt.arg))
...
def _stmt_check_module_objs(ctx: context.Context, stmt: statements.Statement):
    ...
    for sub in stmt.substmts:
        if sub.keyword not in allowed:
            error.err_add(ctx.errors, sub.pos, "AMM_MODULE_OBJS", (keyword_to_str(sub.keyword), sub.arg))
...
def _stmt_check_obj_enum(ctx: context.Context, stmt: statements.Statement):
    ...
    enum_stmt = stmt.search_one((MODULE_NAME, "enum"))
    if not enum_stmt:
        error.err_add(ctx.errors, stmt.pos, "AMM_OBJ_ENUM", (stmt.raw_keyword[1], stmt.arg))
...
def _stmt_check_intlabels(ctx: context.Context, stmt: statements.Statement):
    ...
    if not has_enum and not has_bit:
        ...
        error.err_add(ctx.errors, stmt.pos, "AMM_INTLABELS", ("but not both"))

    seen = set()
    for enum_stmt in stmt.search("enum"):
        val_stmt = enum_stmt.search_one("value")
        if val_stmt is None:
            error.err_add(ctx.errors, enum_stmt.pos, "AMM_INTLABELS_ENUM_VALUE", enum_stmt.arg)
        else:
    ...
    for enum_stmt in stmt.search("bit"):
        ...
        if pos_stmt is None:
            error.err_add(ctx.errors, enum_stmt.pos, "AMM_INTLABELS_BIT_VALUE", enum_stmt.arg)
        else:
    ...
    for obj_kywd in AMM_OBJ_NAMES:
        ...
        for obj_stmt in stmt.search(obj_kywd):
            ...
            if enum_val in seen_enum:
                error.err_add(ctx.errors, obj_stmt.pos, "AMM_OBJ_ENUM_UNIQUE", (obj_kywd[1], enum_stmt.arg))
...
def _stmt_check_documentation(ctx: context.Context, stmt: statements.Statement):
    if stmt.search_one("description") is None:
        error.err_add(ctx.errors, stmt.pos, "AMM_DOC_DESCRIPTION", (keyword_to_str(stmt.keyword), stmt.arg))
