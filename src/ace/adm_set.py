...
class DbRepository(Repository):
    ...
    def get_module_from_handle(self, handle):
        if isinstance(handle[1], int):
            found = (
                self._db_sess.query(models.AdmSource.abs_file_path, models.AdmSource.file_text)
                .filter(models.AdmSource.id == handle[1])
                .one_or_none()
            )
            if found is None:
                raise Repository.ReadError(f"No ADM found with DB ID {handle[1]}")
...
class AdmSet:
    ...
    def __init__(self, cache_dir: str = None):
        ...
        if db_vers != cur_vers:
            LOGGER.info("Recreating cache DB version %s because of old version %s", cur_vers, db_vers)
            self._db_close()
    ...
    def names(self) -> Set[str]:
        ...
        """
        query = self._db_sess.query(models.AdmModule.norm_name).filter(models.AdmModule.norm_name.is_not(None))
    ...
    def __contains__(self, name: str) -> bool:
        ...
        """
        query = self._db_sess.query(models.AdmModule.norm_name).filter(models.AdmModule.norm_name == name)
        return query.count()
    ...
    def get_by_norm_name(self, name: str) -> models.AdmModule:
        ...
        query = self._db_sess.query(models.AdmModule).filter(models.AdmModule.norm_name == name)
    ...
    def get_by_enum(self, enum: int) -> models.AdmModule:
        ...
        query = self._db_sess.query(models.AdmModule).filter(models.AdmModule.enum == enum)
        adm = query.one_or_none()
        if not adm:
    ...
    def load_default_dirs(self) -> int:
        ...
        """
        prefix_list = [xdg_base_dirs.xdg_data_home()] + xdg_base_dirs.xdg_data_dirs()

        adm_dirs = [os.path.join(prefix, "ace", "adms") for prefix in prefix_list]
    ...
    def _is_usable(item: os.DirEntry) -> bool:
        return item.is_file() and item.name.endswith(".yang")
    ...
    def _read_file(self, dec: adm_yang.Decoder, file_path: str, del_dupe: bool) -> models.AdmModule:
        ...
        if src_existing is not None and src_existing.last_modified >= dec.get_file_time(file_path):
            LOGGER.debug("Skipping file %s already loaded from time %s", file_path, src_existing.last_modified)
            mod = self._db_sess.query(models.AdmModule).filter(models.AdmModule.source_id == src_existing.id).one()
            return mod
    ...
    def _post_load(self, adm_new: models.AdmModule, del_dupe: bool):
        ...
        if del_dupe:
            query = self._db_sess.query(models.AdmModule).filter(models.AdmModule.norm_name == adm_new.norm_name)
