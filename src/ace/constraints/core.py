...
class Checker:
    ...
    def check(self, src: models.AdmModule = None):
        ...
        LOGGER.info("Checked %d rules and produced %d issues", check_count, len(allissues))
        return allissues

    def _add_result(
        self, issuelist: List[Issue], check_count: int, cst_name: str, cst: Callable, adm: models.AdmModule
    ):
